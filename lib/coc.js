...
exports.getSuccessRank = function(border, result) {
  if (result == 1) {
    return "**Critical !!!!**"
  }
  if (border / 5 >= result ) {
  ...
  if (border >= 51 && result == 100) {
    return "**Famble !?**"
  }
  if (border <= 50 && result >= 96) {
    return "**Famble !?**"
  }
  return "Failure..."
}
...
exports.getResultFace = function(border, resultFace, result) {
  return "Judge: " + border + "% ... [ 出目 : " + resultFace + " " + exports.getSuccessRank(border, result) + " ]"
}
