...
const COC_PATTERN = /^coc[0-9\+\-\*\/\(\)]+(?:vs[0-9\+\-\*\/\(\)]+)?$/
...
class CocCommand {
  ...
  get rate() {
    ...
    let rate = 0
    if ( match ) {
      rate = Math.floor( math.eval(match[1]) )
    }
    const vsMatch = this.raw.match(/vs([0-9]+(?:[\+\-\*\/\(\)][0-9]+)*)$/)
    if ( vsMatch ) {
      rate = 50 + ( rate - Math.floor( math.eval(vsMatch[1]) ) ) * 5
    }
    return rate
  }
  ...
  run() {
    ...
    const content = `( ${result.savedRate}% ) ${ result.result } \u2192 **${ result.humanize }**`
    let fields = []
    if ( result.rate != Infinity && this.additionalDices != 0 ) {
      fields.push({ name: "Dices", value: result.humanizeDices.join(', ') })
    }
    return new GeneralResponse(content, { fields: fields })
  }
