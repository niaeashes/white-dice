...
class KeyNoCommand {
  ...
  run() {
    ...
    let content = `( ${result.dices.join(' + ')} = ${result.sum} ${result.humanize && `**${result.humanize}** `}) \u2192 ${result.power} ${this.exp} = ${result.damage}`
    let fields = [
      { name: "威力表", value: result.humanizeTable.join(', ') }
    ]
    let nextCommand = null
    if ( result.isCritical ) {
      nextCommand = `${this.raw}+${result.power}`
    }
    return new GeneralResponse(content, { fields, nextCommand } )
  }
...
class KeyNoResult {
  ...
  get isFumble() {
    return this.table[this.sum - 2] == -1
  }
  ...
  get humanizeTable() {
    let table = this.table.slice(0)
    table.shift()
    return table
  }

  get power() {
    return Math.max(this.table[this.sum - 2], 0)
  }
  ...
  get damage() {
    ...
    return math.eval(`${this.power}${this.command.exp}`)
  }
